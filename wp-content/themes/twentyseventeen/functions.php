...
function new_post_type() {
	...
	$labels = array(
		'name'                  => _x( 'New post types', 'Post Type General Name', 'twentyseventeen' ),
		'singular_name'         => _x( 'New post type', 'Post Type Singular Name', 'twentyseventeen' ),
		'menu_name'             => __( 'New Post Types', 'twentyseventeen' ),
		'name_admin_bar'        => __( 'New Post Type', 'twentyseventeen' ),
	...
	$args = array(
		'label'                 => __( 'New post type', 'twentyseventeen' ),
...
function create_jewelry_taxonomies() {
	...
	register_taxonomy( 'jewelry', array( 'new_post_type' ), $args );
}
...
if ( is_home() && $query->is_main_query() )
    $query->set( 'post_type', array( 'post', 'new_post_type' ) );
  return $query;
}
...
class Rational_Meta_Box {
	...
	public function generate_fields( $post ) {
		...
		foreach ( $this->fields as $field ) {
			...
			$db_value = get_post_meta( $post->ID, 'contact_' . $field['id'], true );
			switch ( $field['type'] ) {
				default:
					$input = sprintf(
	...
	public function save_post( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			if ( isset( $_POST[ $field['id'] ] ) ) {
				...
				update_post_meta( $post_id, 'contact_' . $field['id'], $_POST[ $field['id'] ] );
			} else if ( $field['type'] === 'checkbox' ) {
				update_post_meta( $post_id, 'contact_' . $field['id'], '0' );
			}
		}
	}
}
...
//Crete function to display exact number of the posts from the exact author
function shortcode_lista_postova($atts) {
	// pravljenje promenjivih i dodeljivanje defaultnih vrednosti
	   extract(
		   shortcode_atts(
			   array(
				   'pisac'       => 'pleme',
	...
	// Sada mogu da koristim promenjivu $pisac $br_postova
	   $args= array(
		   'author_name'    => $pisac,
		   'posts_per_page' => $br_postova,
	   );
	   $upit = new WP_Query( $args );
   
	// The Loop
	   $naslovi_postova = '';
	   $naslovi_postova .='Izabrali ste da pogledate '.$br_postova. ' clanak od autora '.$pisac.':'.'</br>';
	   if($upit->have_posts()) :
		 while($upit->have_posts()) : $upit->the_post();
		   $naslovi_postova .='<a href="'.get_permalink().'">'.get_the_title().'</a><br>';
		 endwhile;
		 wp_reset_query();
		 return $naslovi_postova;
	   else:
		 $nema_nista = '<p>Zao nam je nema postova sa izbranim autorom '. $pisac.'</p>';
		 return $nema_nista;
	   endif;
   } 

   
function dodavanje_shortcode(){
    add_shortcode('spisak_postova', 'shortcode_lista_postova');
}
add_action( 'init', 'dodavanje_shortcode');

// Include widget code in functions.php
include_once locate_template('/widget.php');
